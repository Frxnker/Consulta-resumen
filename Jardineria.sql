...
SELECT
    (SELECT MAX(precio_venta) FROM producto) AS precio_mas_caro,
    (SELECT MIN(precio_venta) FROM producto) AS precio_mas_barato;
...
SELECT empleado.nombre AS representante_ventas, COUNT(cliente.codigo_cliente) AS numero_clientes
FROM empleado
...
GROUP BY cliente.codigo_cliente, cliente.nombre_cliente, cliente.nombre_contacto, cliente.apellido_contacto;
...
SELECT detalle_pedido.codigo_producto, producto.nombre, SUM(detalle_pedido.cantidad) AS total_unidades_vendidas
FROM detalle_pedido
...
SELECT
    ...
    SUM(detalle_pedido.precio_unidad * detalle_pedido.cantidad) + (SUM(detalle_pedido.precio_unidad * detalle_pedido.cantidad) * 0.21) AS total_facturado_con_iva
FROM detalle_pedido
JOIN producto ON detalle_pedido.codigo_producto = producto.codigo_producto
